...
namespace LOCKSSOMatic\LogBundle\Services;
...
class LoggingService
{
    ...
    private $ignoredClasses = array(
        'LOCKSSOMatic\LogBundle\Entity\LogEntry',
        'LOCKSSOMatic\LogBundle\Services\LoggingService',
    );
    ...
    private function findStackFrame()
    {
        ...
        foreach ($trace as $f) {
            ...
            $class = $f['class'];
            if (preg_match('/LogBundle/', $class) &&
                $class != 'LOCKSSOMatic\LogBundle\Command\ExportLogsCommand') {
                continue;
            }
    ...
    public function export($header = true, $purge = false)
    {
        ...
        $dql = 'SELECT p FROM LOCKSSOMaticLogBundle:LogEntry p';
